...
def eval_policy(policy, env, seed, eval_episodes=10):
	...
	for _ in range(eval_episodes):
		...
		while not done:
			...
			state, reward, done, arrive = eval_env.step(action)
			avg_reward += reward
...
if __name__ == "__main__":
	...
	for t in range(int(args.max_timesteps)):
		...
		# else:
		action[0] = (
			policy.select_action(np.array(state)) + np.random.normal(0, action_linear_max * args.expl_noise, size=action_dim)
		).clip(0, action_linear_max)
		action[1] = (
			policy.select_action(np.array(state)) + np.random.normal(0, action_angular_max * args.expl_noise, size=action_dim)).clip(-action_angular_max, action_angular_max)

		# Perform action
		next_state, reward, done, arrive = env.step(action)
