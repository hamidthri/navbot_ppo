...
from current_rel_theta import reletiveangle
...
class Env():
    ...
    def getGoalDistace(self):
        ...
        goal_distance = math.hypot(rel_dis_x, rel_dis_y)
        # past_rel_theta = reletiveangle(rel_dis_x, rel_dis_y)
        # self.past_rel_theta = past_rel_theta
        self.past_distance = goal_distance

        # return goal_distance, past_rel_theta
    ...
    def getOdometry(self, odom):
        ...
        if rel_dis_x > 0 and rel_dis_y > 0:
            ...
            theta = math.pi
        rel_theta = round(math.degrees(theta))
        # print('rel:{}'.format(rel_theta))
        diff_angle = abs(rel_theta - yaw)
        # print('diff angle:{}'.format(diff_angle))
        # diff_angle = (rel_theta - yaw)
        # print(diff_angle)
        if diff_angle <= 180:
            diff_angle = round(diff_angle, 2)
            # print('first:{}'.format(diff_angle))
        else:
            diff_angle = round(360 - diff_angle, 2)
            # print('second: {}'.format(diff_angle))
    ...
    def setReward(self, done, arrive):
        rel_dis_x = self.goal_position.position.x - self.position.x
        rel_dis_y = self.goal_position.position.y - self.position.y
        current_distance = math.hypot(rel_dis_x, rel_dis_y)
        current_rel_theta = reletiveangle(rel_dis_x, rel_dis_y)
        # print('past: {} and current: {}'.format(self.past_rel_theta, current_rel_theta))
        ang_rate = self.past_rel_theta - current_rel_theta
        # print('ang_rate{}'.format(ang_rate))
        distance_rate = (self.past_distance - current_distance)

        reward = 500.*distance_rate
        self.past_distance = current_distance
        self.past_rel_theta = current_rel_theta
        ...
        if arrive:
            ...
            try:
                ...
                # self.goal_position.position.y = -2
