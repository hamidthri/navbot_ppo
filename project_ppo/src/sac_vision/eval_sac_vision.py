...
def evaluate_vision_sac(
    ...
    """
    Evaluate trained vision SAC agent
    
    Args:
        model_path: Path to saved model checkpoint
        num_episodes: Number of evaluation episodes
        max_steps: Maximum steps per episode
        backbone_name: Vision backbone architecture
        render: Whether to render (Gazebo GUI should be running)
    """
    rospy.init_node('sac_vision_eval')
    
    print(f"\n{'='*60}")
    print(f"Vision SAC Evaluation")
    print(f"{'='*60}")
    ...
    env = Env(is_training=False, use_vision=True)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Evaluate Vision SAC Agent')
    ...
    parser.add_argument('--backbone', type=str, default='resnet18',
                        choices=['resnet18', 'resnet50', 'simplecnn'],
